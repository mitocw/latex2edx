...
class Test_Abox(unittest.TestCase):
    ...
    def test_abox_mc_ut1(self):
        ...
        print(ab.xmlstr)
        print("ab tests = %s" % ab.tests)
        assert('choicegroup' in ab.xmlstr)
        if 0:
            # should be no tests in ab?
            print("ab.tests[0]['responses']=%s" % ab.tests[0]['responses'])
    ...
    def test_abox_custom_ut2(self):
        ...
        print("ab.tests[0]['box_indexes'] = %s" % list(ab.tests[0]['box_indexes']))
        assert(ab.tests[0]['box_indexes'][0] == (0,0))
